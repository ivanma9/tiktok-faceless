...
RECOVERY_SUGGESTIONS: dict[str, str] = {
    ...
    "TikTokAuthError": (
        "TikTok authentication failed — refresh access token in AccountConfig"
    ),
    "TikTokAPIError": (
        "TikTok API error — check TikTok developer dashboard for service status"
    ),
    "ElevenLabsError": (
    ...
    "MissingProduct": (
        "No product selected — run research agent to populate niche products"
    ),
    "MissingScript": (
        "No script available — script agent must run before production"
    ),
    "MissingVideo": (
        "No rendered video available — production agent must run before publishing"
    ),
    "LLMError": (
        "LLM API error — check Anthropic API key or retry on next cycle"
    ),
